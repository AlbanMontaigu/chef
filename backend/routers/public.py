...
from .. import config, content, db, mailer
...
def create_booking(payload: BookingIn, background: BackgroundTasks) -> dict:
    ...
    with db.transaction() as conn:
        ...
        cur = conn.execute(
            ...
            INSERT INTO bookings
                (ref, slot_id, name, email, phone, address, guests, formula, message,
                 status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'confirmed', ?)
            """,
            (ref, payload.slot_id, payload.name, payload.email, payload.phone,
             payload.address, payload.guests, payload.formula, payload.message, now),
        )
    ...
    booking = {
        ...
        "formula": payload.formula, "message": payload.message,
    }
